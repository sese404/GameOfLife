...
function game_of_life!(fields::Matrix{UInt8},iterations::UInt64)
    
    rows = size(fields, 1)
    columns = size(fields, 2)
    new_fields = zeros(UInt8, rows, columns)
    cond = 0
...
function play(iterations::UInt64)
    rows::UInt8 = 10
    columns::UInt8 = 10
    fields = zeros(UInt8, rows, columns)
    fields[6:7,4] .= 1
    fields[6:7,5] .= 1
    fields[6:7,6] .= 1
    fields[6:7,7] .= 1
